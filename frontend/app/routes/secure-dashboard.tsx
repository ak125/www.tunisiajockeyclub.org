...
export const loader: LoaderFunction = async ({ request }: LoaderFunctionArgs) => {
  // Requérir une authentification (super_admin ou admin)
  const user = await requireAuth(request, ['super_admin', 'admin']);
  ...
  try {
    ...
    const recentActivity = [
      ...
      },
      {
        id: '4',
        type: 'security',
        description: `Connexion administrateur: ${user.username}`,
...
export default function SecureExecutiveDashboard() {
  const { user, systemHealth, metrics, recentActivity, upcomingEvents } = useLoaderData<typeof loader>();
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h1 className="text-xl font-bold text-slate-800">Dashboard Sécurisé</h1>
                  <p className="text-sm text-slate-600">
                    Connecté en tant que <span className="font-semibold">{user.firstName} {user.lastName}</span>
                  </p>
                </div>
              </div>
              
              <div className="flex items-center space-x-2 ml-8">
                <ExecutiveBadge variant={user.role === 'super_admin' ? 'ministerial' : 'authority'} size="sm">
            ...
            <div className="flex items-center space-x-4">
              ...
              <form action="/logout" method="post">
                <ExecutiveButton variant="ministerial" size="sm">
                  🚪 Déconnexion
                </ExecutiveButton>
              </form>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-4">
        ...
        >
          <div className="flex items-center space-x-3">
            ...
            <div>
              <p className="text-green-800 font-semibold">Session Sécurisée Active</p>
              <p className="text-green-600 text-sm">
                Dernière connexion: {new Date(user.lastLogin || new Date()).toLocaleString('fr-FR')}
              </p>
            </div>
          </div>
        </motion.div>
      ...
      <main className="max-w-7xl mx-auto px-6 pb-8">
        {/* Rest of the dashboard content - same as before but with security features */}
        {/* Filters and Search */}
        <div className="mb-8">
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200/60">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-slate-800">Contrôles Avancés</h2>
          ...
          <div className="lg:col-span-2">
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200/60">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-xl font-semibold text-slate-800">Activité Sécurisée</h2>
